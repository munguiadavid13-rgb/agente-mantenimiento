...
dashboard.py
------------
Dashboard web EN VIVO del agente de mantenimiento predictivo.

Levanta un pequeno servidor web (Flask) en tu computadora. Cada vez que
abres o recargas la pagina, vuelve a leer el Excel y muestra el estado
actual de TODOS los equipos (no solo los que tienen alerta), con sus
...
Reutiliza el mismo modelo y los mismos graficos que usa el agente, asi
que el dashboard y las notificaciones siempre muestran lo mismo.
...
.actualizar { display:inline-block; background:#0d6efd; color:#fff;
...
def construir_pagina():
    ...
    H.append('<div class="barra">'
             ...
             f'<p>Estado en vivo &middot; actualizado: {hoy}</p></div>')
    ...
    H.append('<a class="actualizar" href="/">&#128260; Actualizar</a>')

    # una tarjeta por equipo (TODOS, con alerta o no)
    for e in equipos:
        en_alerta = e.codigo in codigos_alerta
        clase = "rojo" if en_alerta else "verde"
        v = e.ultima_visita()
        H.append(f'<div class="equipo {clase}">')
...
def index():
    try:
        return construir_pagina()
    except Exception as e:
        return (f"<h2>Error al generar el dashboard</h2><pre>{e}</pre>"
                "<p>Revisa que el archivo Excel exista y no este abierto en modo "
                "exclusivo.</p>"), 500


if __name__ == "__main__":
    print("=" * 55)
    print(" Dashboard del Agente de Mantenimiento")
